...
export default function ANASkiplagClient() {
  const [flights, setFlights] = useState<ANAFlight[]>([]);
  const [koreanAirFlights, setKoreanAirFlights] = useState<ANARoute[]>([]);
  ...
  const [asiaFilter, setAsiaFilter] = useState<string>('any');
  ...
  const getDateDifference = (departDate: string, arriveDate: string) => {
    try {
      ...
      const departMatch = departDate.match(/(\d{4})-(\d{2})-(\d{2})/);
      const arriveMatch = arriveDate.match(/(\d{4})-(\d{2})-(\d{2})/);
      
      if (!departMatch || !arriveMatch) return 0;
      
      const departDateOnly = new Date(parseInt(departMatch[1]), parseInt(departMatch[2]) - 1, parseInt(departMatch[3]));
      const arriveDateOnly = new Date(parseInt(arriveMatch[1]), parseInt(arriveMatch[2]) - 1, parseInt(arriveMatch[3]));
  ...
  const filteredFlights = flights.filter(flight => {
    ...
    // Asia filtering - only HND/NRT
    const matchesAsiaFilter = (() => {
      if (asiaFilter === 'any') return true;
      
      const tokyoAirports = ['HND', 'NRT'];
      
      if (asiaFilter === 'to_asia') {
        return tokyoAirports.includes(flight.destination_airport);
      }
      
      if (asiaFilter === 'from_asia') {
        return tokyoAirports.includes(flight.origin_airport);
      }
    ...
    return matchesSearch && matchesDateRange && matchesAsiaFilter && matchesSelectedFlight;
  });
  ...
  const callKoreanAirAPI = async (flightData: ANAFlight) => {
    try {
      ...
      const response = await fetch('https://api.bbairtools.com/api/NH-F-outbound', {
        method: 'POST',
        headers: {
      ...
      console.log('ANA API response:', data);
      
      // Store the ANA routes data with flights
      ...
      console.error('Error calling ANA API:', error);
      setKoreanAirFlights([]);
    } finally {
  ...
  const calculateMinValue = (route: ANARoute, flightIndex: number) => {
    const f1 = route.f1 || 0;
    const y2 = route.y2 || 0;
    const j2 = route.j2 || 0;
    const f2 = route.f2 || 0;
      ...
      <div className="max-w-[1000px] mx-auto px-2 py-4">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Loading ANA flights...</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="max-w-[1000px] mx-auto px-4 py-6">
      ...
      <div className="mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="flex flex-col justify-center">
            ...
            <Select value={asiaFilter} onValueChange={setAsiaFilter}>
              <SelectTrigger className="w-full dark:bg-gray-800 dark:border-gray-600 dark:text-gray-100">
                <SelectValue placeholder="Tokyo Route" />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="any">Any</SelectItem>
                <SelectItem value="to_asia">To Tokyo (HND/NRT)</SelectItem>
                <SelectItem value="from_asia">From Tokyo (HND/NRT)</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
                    ...
                    <div className="flex-1">
                      ...
                      <Button
                        onClick={async () => {
                          try {
                            setLoading(true);
                            const response = await fetch('https://api.bbairtools.com/api/seats-aero-ana');
                            if (response.ok) {
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    <span className="flex items-center gap-1">
                      ...
                      >
                        {(() => {
                          const tokyoAirports = ['HND', 'NRT'];
                          if (tokyoAirports.includes(flight.origin_airport)) {
                            return 'Disabled';
                          }
      ...
      {(koreanAirFlights.length > 0 || koreanAirLoading) && (
          ...
          {koreanAirLoading ? (
            <div className="flex items-center justify-center min-h-[200px]">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">Loading ANA routes...</p>
              </div>
            </div>
          ) : (
            <div className="flex flex-col gap-4">
        ...
        <p className="text-gray-600 dark:text-gray-300">Book your complete itinerary with ANA</p>
              </div>
                              ...
                              <div className="flex-1">
                                ...
                                <div className="text-sm text-gray-600 dark:text-gray-400">
                                  {formatDate(selectedKoreanFlight.date)} • {(() => {
                                    ...
                                    return flightData?.FlightNumbers || 'ANA Flight';
                                  })()}
                                </div>
                              </div>
                              <div className="text-right">
